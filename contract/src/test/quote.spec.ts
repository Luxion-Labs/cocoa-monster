...
describe("quoteAmountOut", () => {
  ...
  it("matches the contract's reserve update for a YES buy", () => {
    ...
    const amountOut = quoteAmountOut(reserveYes, reserveNo, Side.YES, collateral);

    // Plug into the simulator: this exact (collateral, amountOut) must
    ...
    sim.buy(Side.YES, collateral, amountOut);
    const after = sim.ledger();
    expect(after.reserveYes).toBe(reserveYes - amountOut);
    expect(after.reserveNo).toBe(reserveNo + collateral);
  });
  ...
  it("matches for a NO buy", () => {
    ...
    const collateral = 250n;
    const amountOut = quoteAmountOut(1000n, 1000n, Side.NO, collateral);
    sim.buy(Side.NO, collateral, amountOut);
    const l = sim.ledger();
    expect(l.reserveYes).toBe(1000n + collateral);
